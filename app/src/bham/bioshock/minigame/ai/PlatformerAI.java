 ...
 * abstract class and implements methods execute() and updateStack(). execute() contains the
 * functions of the state updateStack() determines when/if and how the next state is added to the
 * stack.
 */
public class PlatformerAI extends MinigameAI {
  ...
  /*
   * These distances were determined through trial and error by logging the distance of human player
   * as it approaches platforms.
   */
  ...
  /**
   * currentPlatformIndex indicates which platform (in the path) the cpu must reach next 0 indicates
    ...
    void execute() {
       ...
       */
        ...
        for (int i = 1; i < all.size(); i++) {
          if (!(all.get(i).getId().equals(pathToGoal.get(currentPlatformIndex).getId()))
              && (all.get(i).getPos().sqDistanceFrom(currentPosition) < distance)) {
            nearest = all.get(i);
          }
        }
  ...
  private class MoveTowardsPlatform extends CPUState {
    ...
    void updateStack() {
      /*
       * Add a state to make the astronaut jump Do not remove the current state because the
    ...
    void updateStack() {
      ...
      if (astronautOn.isPresent()) {
        ...
        if (astronautOn.get().getId().equals(nextPlatform.getId())) {
          ...
          /*
           * The stack will revert back to moveTowardsPlatform to continue moving to the target
           * platform
           */
